def run(train_ids, val_ids, test_ids, track_dir, output_dir, result_summary):
    """
    Run the LSTM baseline on the given train, validation, and test sets.

    Args:
        train_ids (list): List of training example IDs.
        val_ids (list): List of validation example IDs.
        test_ids (list): List of test example IDs.
        track_dir (Path): Directory containing the trajectory data.
        output_dir (Path): Directory to save the results.
        result_summary (dict): Dictionary to store the results summary.
    """
    print("\n--------- Running LSTM Baseline ----------------------\n")

    # TODO: Implement the LSTM baseline here

    print("\n--------- LSTM Baseline Done! ------------------------\n")