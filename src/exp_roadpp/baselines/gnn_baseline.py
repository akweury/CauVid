...
"""
The GNN baseline model is implemented in this file.
...
Every agent in a segment is encoded as a single node: its class is embedded and the
...
embedding space. All agent nodes in a segment (which may be empty) are connected to
each other and to a learnable global node, forming a fully-connected graph. A stack of
mean-aggregator message-passing (GraphSAGE-style) layers updates every node from its
neighbors, and the final av_action_id is predicted from the global node's representation.
"""
...
class _GraphAggLayer(nn.Module):
    """A single mean-aggregator (GraphSAGE-style) message-passing layer over a
    fully-connected graph: every node is updated from itself and the mean of all
    other valid (non-padded) nodes in the same graph."""
    ...
    def forward(self, nodes, valid_mask):
        # nodes: (B, M, D); valid_mask: (B, M) with 1.0 for valid nodes, 0.0 for padded ones.
        masked_nodes = nodes * valid_mask.unsqueeze(-1)
        neighbor_sum = masked_nodes.sum(dim=1, keepdim=True)
        neighbor_count = valid_mask.sum(dim=1, keepdim=True).clamp(min=1.0).unsqueeze(-1)
        neighbor_mean = neighbor_sum / neighbor_count
        neighbor_mean = neighbor_mean.expand(-1, nodes.shape[1], -1)

        updated = torch.relu(self.self_lin(nodes) + self.neigh_lin(neighbor_mean))
        updated = self.norm(updated)
        updated = self.dropout(updated)
        return updated * valid_mask.unsqueeze(-1)


class _GNNNet(nn.Module):
    def __init__(self, vocab, embed_dim=16, num_layers=2, dropout=0.4):
        super().__init__()
        ...
        self.global_node = nn.Parameter(torch.zeros(1, 1, embed_dim))
        nn.init.normal_(self.global_node, std=0.02)

        self.layers = nn.ModuleList([_GraphAggLayer(embed_dim, dropout) for _ in range(num_layers)])
        self.dropout = nn.Dropout(dropout)
    ...
    def forward(self, agent_class_ids, action_multihot, loc_multihot, padding_mask):
        ...
        global_tokens = self.global_node.expand(batch_size, -1, -1)
        nodes = torch.cat([global_tokens, agent_tokens], dim=1)

        global_mask = torch.zeros(batch_size, 1, dtype=torch.bool, device=nodes.device)
        full_padding_mask = torch.cat([global_mask, padding_mask], dim=1)
        valid_mask = (~full_padding_mask).float()

        for layer in self.layers:
            nodes = layer(nodes, valid_mask)

        global_output = self.dropout(nodes[:, 0, :])
        return self.classifier(global_output)
...
class GNNModel:
    ...
    def __init__(self, vocab, embed_dim=16, num_layers=2, dropout=0.4,
        ...
        self.model = _GNNNet(vocab, embed_dim=embed_dim, num_layers=num_layers, dropout=dropout).to(self.device)
    ...
    def train(self, train_facts, val_facts):
        """
        Train the GNN model on the training facts and validate on the validation facts.
        ...
        """
        if not train_facts:
            print("No training facts provided; skipping GNN training.")
            return
        ...
        for epoch in range(1, self.max_epochs + 1):
            ...
            if val_loader is not None:
                ...
                print(f"[GNN] Epoch {epoch}: train_loss={train_loss:.4f} train_acc={train_acc:.4f} "
    ...
    def evaluate(self, test_facts, dataset_labels):
        """
        Evaluate the GNN model on the test facts.
        ...
        print(f"[GNN] Test accuracy: {test_accuracy:.4f}, F1 macro: {test_f1_macro:.4f}")
...
def prepare_dataset(train_facts, val_facts, test_facts):
    """
    Prepare the dataset for the GNN model, i.e. compute the vocabulary sizes
...
def run(all_facts, dataset_labels, output_dir, result_summary, device):
    """
    Run the GNN baseline on the given train, validation, and test sets.
    ...
    model_output_dir = output_dir / "gnn"
    os.makedirs(model_output_dir, exist_ok=True)

    train_facts, val_facts, test_facts = all_facts["train"], all_facts["val"], all_facts["test"]
    gnn_results = {}
    gnn_model_file = model_output_dir / "gnn_model.pth"  # Path to save the trained model
    gnn_result_file = model_output_dir / "gnn_results.json"  # Path to save the results
    if gnn_model_file.exists() and gnn_result_file.exists():
        print("GNN model and results already exist. Loading them...")
        # Load the model and results if they already exist
        gnn_results = utils_data.load_json(gnn_result_file)
    else:
        vocab = prepare_dataset(train_facts, val_facts, test_facts)
        gnn_model = GNNModel(vocab, device=device)  # Initialize the GNN model
        gnn_model.train(train_facts, val_facts)  # Train the model
        gnn_model.save(gnn_model_file)  # Save the trained model weights
        gnn_results = gnn_model.evaluate(test_facts, dataset_labels)  # Evaluate on test set
        utils_data.save_json(gnn_results, gnn_result_file)  # Save the results
    result_summary["gnn"] = gnn_results
    print("\n--------- GNN Baseline Done! ------------------------\n")