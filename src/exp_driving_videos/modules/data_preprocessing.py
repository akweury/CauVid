...
def get_frames_root() -> Path:
    """Return the default extracted-frames root for driving_mini."""
    return config.get_dataset_path("driving_mini") / "frames"


def get_depth_maps_root() -> Path:
    """Return the default depth-maps root for driving_mini."""
    return config.get_dataset_path("driving_mini") / "depth_maps"


def _depth_map_path_for_frame(depth_video_dir: Union[str, Path], frame_name: str) -> Path:
    """Resolve the expected Depth Anything npz path for a frame file name."""
    depth_video_dir = Path(depth_video_dir)
    stem = Path(frame_name).stem
    return depth_video_dir / f"{stem}_depth.npz"


def ensure_video_depth_maps(
    video_id: str,
    frames_root: Union[str, Path, None] = None,
    depth_root: Union[str, Path, None] = None,
    model_name: str = "depth-anything/DA3-Large",
    batch_size: int = 4,
    device: str = "auto",
    ...
    Returns the per-video depth directory.
    """
    frames_root = Path(frames_root) if frames_root is not None else get_frames_root()
    depth_root = Path(depth_root) if depth_root is not None else get_depth_maps_root()

    frame_dir = frames_root / video_id
    depth_dir = depth_root / video_id
    if not frame_dir.exists():
        raise FileNotFoundError(f"Frame directory not found for {video_id}: {frame_dir}")
...
def estimate_3d_positions_for_frame(
    bboxes: List[List[float]],
    depth_map_path: Union[str, Path],
    frame_path: Union[str, Path],
) -> List[List[float]]:
    """Estimate simple camera-frame 3D centers from 2D boxes and a depth map.

    Uses the median non-zero depth inside each box and a pinhole-camera-style
    backprojection with focal length approximated from image size.
    """
    ...
    frame_height, frame_width = img.size[1], img.size[0]
    depth_map = utils.load_depth_npz(depth_map_path, target_size=(frame_height, frame_width)).numpy()

    def get_object_depth(depth: np.ndarray, bbox: List[float]) -> float:
        x1, y1, x2, y2 = [int(round(v)) for v in bbox]
        x1 = max(0, min(x1, depth.shape[1] - 1))
        x2 = max(x1 + 1, min(x2, depth.shape[1]))
        y1 = max(0, min(y1, depth.shape[0] - 1))
        y2 = max(y1 + 1, min(y2, depth.shape[0]))
    ...
    for bbox in bboxes:
        x1, y1, x2, y2 = bbox
        z_val = get_object_depth(depth_map, bbox)
        u = (x1 + x2) / 2.0
        v = (y1 + y2) / 2.0
        x_val = (u - cx) * z_val / fx
        y_val = (v - cy) * z_val / fy
        positions_3d.append([float(x_val), float(y_val), float(z_val)])

    return positions_3d


def prepare_video_3d_positions(
    video_result: dict,
    frames_root: Union[str, Path, None] = None,
    depth_root: Union[str, Path, None] = None,
    model_name: str = "depth-anything/DA3-Large",
    ...
    frames_root = Path(frames_root) if frames_root is not None else get_frames_root()
    depth_dir = ensure_video_depth_maps(
        video_id=video_id,
    ...
    for frame_pos, frame_entry in enumerate(frame_entries, start=1):
        ...
        updated_frame = dict(frame_entry)
        updated_frame["depth_map_path"] = str(depth_map_path)
        if not frame_path.exists() or not depth_map_path.exists():
            missing_assets = []
            if not frame_path.exists():
                missing_assets.append("frame")
            if not depth_map_path.exists():
            ...
            logger.warning(
                "Skipping 3D position estimation for %s %s (%s missing)",
                video_id,
                frame_name,
                ", ".join(missing_assets),
            )
    ...
    result["frames"] = frames_with_3d
    result["num_frames_missing_3d_assets"] = skipped_frames
    return result
