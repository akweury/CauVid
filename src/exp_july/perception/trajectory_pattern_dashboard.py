...
def _ablations(records):
    ...
    for record in records:
        ...
        unresolved+=not str(record.get("resolution_status")).startswith("validated")
...
<body><header><h1>Step 8C Prior-Guided Statistical Repair Dashboard <span class='badge warn'>READ ONLY</span></h1><div class='filters'>
...
<section class='panel wide'><h2>Semantic cohort and calibrated repair policy</h2><div id='cohortPolicy'></div></section>
...
function isCase(r,c){if(c==='disagreement')return r.LLM_preferred_pattern!==r.validated_pattern;if(c==='unresolved')return !String(r.resolution_status).startsWith('validated');if(c==='hard_failure')return r.candidate_repairs.some(x=>x.symbolic_verdict==='reject');return true}
...
function renderList(){$('trackList').innerHTML=S.rows.map((r,i)=>`<div class="track ${i===S.index?'active':''}" onclick="S.index=${i};renderList();render()"><b>${esc(r.video_id)} / track ${r.track_id}</b><br><span class="badge ${String(r.resolution_status).startsWith('validated')?'good':'warn'}">${esc(r.resolution_status)}</span>${r.LLM_preferred_pattern!==r.validated_pattern?'<span class="badge bad">LLM ≠ validated</span>':''}</div>`).join('')||'No matching tracks'}
function current(){return S.rows[S.index]}function render(){const r=current();$('position').textContent=S.rows.length?`${S.index+1}/${S.rows.length}`:'0/0';if(!r)return;renderAlerts(r);renderMedia(r);renderSignal();renderCohortPolicy(r);renderResidual(r);renderCandidates(r);renderSymbolic(r);renderDecision(r);renderLLM()}
function renderAlerts(r){let a=[];if(r.LLM_preferred_pattern!==r.validated_pattern)a.push(`<div class="alert bad">LLM preferred <b>${esc(r.LLM_preferred_pattern)}</b>, validated pattern is <b>${esc(r.validated_pattern)}</b>.</div>`);if(!String(r.resolution_status).startsWith('validated'))a.push('<div class="alert">No candidate passed all hard constraints; original trajectory was preserved.</div>');if(r.resolution_status==='validated_no_repair')a.push('<div class="alert good">The original trajectory passed validation; no repair was required.</div>');const sel=r.selected_candidate||{},ss=sum((sel.post_repair_pattern_scores||{})[sel.pattern_id]||{});if(r.candidate_repairs.some(x=>x.symbolic_verdict==='reject'&&sum((x.post_repair_pattern_scores||{})[x.pattern_id]||{})<ss))a.push('<div class="alert bad">A lower-residual candidate was rejected by symbolic constraints.</div>');$('alerts').innerHTML=a.join('')}
...
function renderCohortPolicy(r){const p=r.cohort_operator_plan||{},rule=r.activated_rule||{},cal=p.calibration||{};$('cohortPolicy').innerHTML=`<div class="grid"><div><b>Cohort:</b> ${esc(r.trajectory_cohort_id)}</div><div><b>Activated rule:</b> ${esc(rule.rule_id)}</div><div><b>Operator:</b> <span class="good">${esc(p.operator||'no_repair')}</span></div><div><b>Policy fingerprint:</b> ${esc((r.provenance||{}).cohort_policy_fingerprint)}</div></div><h3>Static metadata</h3><pre>${esc(JSON.stringify(r.cohort_static_metadata||{},null,2))}</pre><h3>Compiled rule</h3><pre>${esc(JSON.stringify(rule,null,2))}</pre><h3>Calibrated parameters and validation</h3><pre>${esc(JSON.stringify({parameters:p.calibrated_parameters||{},anomaly_types:p.anomaly_types||[],apply_policy:p.apply_policy,calibration:cal},null,2))}</pre>`}
...
function renderDecision(r){$('decision').innerHTML=`<b>LLM role:</b> cohort rule and operator proposal only<br><b>Deterministic repair applied:</b> ${r.repair_applied?'yes':'no; original preserved'}<br><b>Validated:</b> <span class="good">${esc(r.validated_pattern)}</span><br><b>Status:</b> ${esc(r.resolution_status)}<br><b>Reason:</b> ${esc(r.final_selection_reason)}<h3>Frozen policy processing</h3><pre>${esc(JSON.stringify(r.llm_processing||{},null,2))}</pre><h3>Provenance</h3><pre>${esc(JSON.stringify(r.provenance,null,2))}</pre>`}
