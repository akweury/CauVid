...
def _enabled_candidates(result,a,audit):
 ...
 points=[p for p in audit.get("points",[]) if p.get("axis")==a and str(p.get("video_id",""))==str(result.get("video_id","")) and p.get("enabled") and int(p.get("plateau_id",-1)) in plateaus]
 out=[]
 for point in points:
  plateau=plateaus[int(point["plateau_id"])]
  out.append({"threshold_n":float(plateau["midpoint_n"]),"confidence":None if point.get("confidence") is None else float(point["confidence"]),"plateau_id":int(plateau["plateau_id"]),"selection":"enabled_plateau","display_only":True,"segments":plateau.get("segments",[])})
...
def _candidate_stack(im,box,result,audit,indices,now):
 import cv2
 x,y,w,h=box;groups=[("VX | RIGHT · STRAIGHT · LEFT","vx",_enabled_candidates(result,"vx",audit)),("VZ | BACKWARD · STATIC · FORWARD","vz",_enabled_candidates(result,"vz",audit))];total=sum(max(1,len(rows)) for _,_,rows in groups);header_h=32;pitch=min(92,max(18,int((h-header_h*len(groups))/max(1,total))))
 cursor=y
 for title,axis,rows in groups:
  cv2.putText(im,f"{title} | enabled={len(rows)}",(x,cursor+20),cv2.FONT_HERSHEY_DUPLEX,.53,(255,255,255),1,cv2.LINE_AA);cursor+=header_h
  if not rows:
...
def render_eval_signal_segmentation_chart(result,audit,output_path):
 """Render every enabled vx/vz threshold candidate as a k-by-2 matrix."""
 import matplotlib
 matplotlib.use("Agg")
 import matplotlib.pyplot as plt
 import numpy as np
 path=Path(output_path);path.parent.mkdir(parents=True,exist_ok=True);frames=list(result.get("frames",[]));candidates={"vx":_enabled_candidates(result,"vx",audit),"vz":_enabled_candidates(result,"vz",audit)};k=max(1,len(candidates["vx"]),len(candidates["vz"]))
 ...
 for column,(axis,labels) in enumerate((("vx",("right","straight","left")),("vz",("backward","static","forward")))):
  for row in range(k):
   ...
   if row>=len(candidates[axis]):
    ax.set_facecolor("#f1f3f5");ax.text(.5,.5,f"No enabled {axis.upper()} candidate for row {row+1}",transform=ax.transAxes,ha="center",va="center",color="#777777",fontsize=11);ax.set_xticks([]);ax.set_yticks([])
    continue
   ...
   for segment_index,segment in enumerate(candidate.get("segments",[])):
    ...
    ax.axvspan(start-.5,end+.5,color=PLOT_COLORS.get(state,PLOT_COLORS["unavailable"]),alpha=.2,label=state if not any(str(previous.get("state"))==state for previous in candidate["segments"][:segment_index]) else None,zorder=0)
   ...
   ax.set_title(f"{axis.upper()} candidate {row+1}/{len(candidates[axis])} | threshold N={threshold:.5g} | confidence={confidence}",fontweight="bold")
 ...
 fig.suptitle(f"Step 7A all enabled threshold segmentations | video={result.get('video_id','')} | rows={k}",fontsize=16,fontweight="bold")
 fig.savefig(path,dpi=170);plt.close(fig)
 return {"status":"rendered","path":str(path),"layout":"k_by_2_all_enabled_threshold_segmentations","num_rows":k,"vx_enabled_candidates":candidates["vx"],"vz_enabled_candidates":candidates["vz"]}
...
def render_axis_segmentation_mp4(result,ego_video,audit,output_path,fps=10.):
 ...
 path=Path(output_path);path.parent.mkdir(parents=True,exist_ok=True);vx=_candidate(result,"vx",audit);vz=_candidate(result,"vz",audit);W,H,C1,C2=1920,1080,1000,1440;scatter_panel=_scatter_panel(audit,result.get("video_id",""),(W-C2-40,H-48));wr=cv2.VideoWriter(str(path),cv2.VideoWriter_fourcc(*"mp4v"),max(1.,float(fps)),(W,H))
 ...
 for i,f in enumerate(frames):
  ...
  im[24:24+scatter_panel.shape[0],C2+20:C2+20+scatter_panel.shape[1]]=scatter_panel
  wr.write(im)
