...
def main():
    ...
    if observation_ids:
        ...
        obs_matrix = pipeline.process_observation(first_observation)
        
        # Local import to avoid circular dependency
        from causal_workspace.simple_interface import process_and_get_matrix_v2
        causal_relationships = process_and_get_matrix_v2(first_observation)
        
        # Print some results using the correct variable name
        time_series_matrix = obs_matrix['time_series_matrix']
        logger.info(f"Processed {time_series_matrix.num_frames} frames")
        logger.info(f"Found {len(time_series_matrix.object_tracks)} object tracks")
        
        # Show summary of first frame
        if 0 in time_series_matrix.matrix:
            frame_data = time_series_matrix.matrix[0]
            logger.info(f"Frame 0 has {len(frame_data)} objects")
            for obj_id, obj_data in frame_data.items():
                logger.info(f"  Object {obj_id}: position=({obj_data.get('position_x', 'N/A')}, {obj_data.get('position_y', 'N/A')})")
