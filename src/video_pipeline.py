...
def initialize_pipeline(dataset_name):
    """Initialize the video processing pipeline.

    Returns:
        tuple: (pipeline, observation_ids, intervention_ids)
    """
    ...
    inv_ids = pipeline.frame_loader.get_intervention_ids()
    ...
    return pipeline, observation_ids, inv_ids
...
def log_obs_summary(obs_id: str, processed_observation: Dict[str, Any]):
    ...
    if not isinstance(processed_observation, dict):
        print(
            f"   Type: {type(processed_observation).__name__}, Value: {processed_observation}")
        return
            ...
            for attr_name in dir(value):
                if not attr_name.startswith('_') and not callable(getattr(value, attr_name)):
                    ...
                    if attr_value is not None:
                        ...
                        print(
                            f"     └─ {attr_name:16s}: {attr_type:13s} | {attr_info}")
            continue
...
def process_single_observation(pipeline: VideoPipeline, obs_id: str) -> Dict[str, Any]:
    ...
    symbolic_facts = facts_extractor.analyze_facts_in_results(
...
def process_single_intervention_logic(pipeline: VideoPipeline, inv_id: str, observed_rules: Dict[str, Any]) -> Dict[str, Any]:
    """Process a single intervention observation through the pipeline.

    Args:
        pipeline: Initialized VideoPipeline instance
        inv_id: Intervention Observation ID to process
        observed_rules: Dictionary of observed rules from normal observations

    Returns:
        Dictionary containing processing results
    """
    logger.info(f"Processing intervention observation: {inv_id}")

    # Process the intervention observation
    inv_raw = pipeline.process_observation(
        observation_id=inv_id,
        obs_type="intervention",
        include_ground_truth=True,
        extract_features=False,
        analyze_bonds=False
    )
    frames_data = {
        "width": inv_raw["frames_data"][0].frame_size[0],
        "height": inv_raw["frames_data"][0].frame_size[1]
    }

    # Log results summary
    log_observation_summary(inv_id, inv_raw)

    from logic.facts import FactsExtractor
    facts_extractor = FactsExtractor()
    kinematics_facts = facts_extractor.analyze_facts_in_results(inv_raw)

    # smooth the intervention data
    ts_matrix = list(inv_raw["time_series_matrix"].matrix.values())
    processed_ts_matrix = data_process.window_smooth(ts_matrix, window_size=3)

    from logic.clauses import make_predictions
    predictions, violation_events = make_predictions(
        observed_rules, processed_ts_matrix, frames_data)

    return predictions, violation_events
...
def process_single_observation_logic(pipeline: VideoPipeline, obs_id: str) -> Dict[str, Any]:
    ...
    frames_data = obs_raw["frames_data"]
    processed_ts_matrix = data_process.window_smooth(ts_matrix, window_size=3)

    # Extract symbolic facts
    from logic.clauses import learn_rules
    symbolic_facts = learn_rules(frames_data, processed_ts_matrix)
...
def log_observation_summary(obs_id: str, processed_observation: Dict[str, Any]):
    ...
    if detected_objects == 0:
        detected_objects = processed_observation.get(
    ...
    if "classified_bonds" in processed_observation and processed_observation["classified_bonds"]:
        bond_types = processed_observation["classified_bonds"].get(
            "bond_types", {})
    ...
    if symbolic_facts and isinstance(symbolic_facts, dict):
        ...
        total_facts = sum(len(facts) if isinstance(
            facts, (list, dict)) else 1 for facts in symbolic_facts.values())
        print(
            f"   • Symbolic Facts: ✓ {len(fact_categories)} categories, {total_facts} total facts")
    ...
    if causal_edges:
        ...
        print(
            f"   • Causal Analysis: ❌ Failed - {processed_observation.get('error', 'Unknown error')}")
    else:
...
def perform_cross_observation_analysis(all_results: Dict[str, Any], all_causal_graphs: Dict[str, List]) -> Dict[str, Any]:
    ...
    try:
        ...
        prediction_results = build_prediction_models(
            all_results, all_causal_graphs)
...
def log_final_summary(all_results: Dict[str, Any], all_causal_graphs: Dict[str, List]):
    ...
    total_causal_edges = sum(len(edges)
                             for edges in all_causal_graphs.values())

    logger.info("=== PIPELINE COMPLETION SUMMARY ===")
    logger.info(f"✅ Processed {total_scenarios} observations")
    logger.info(
        f"✅ Discovered {total_causal_edges} total causal relationships")
...
def process_intervention_videos(pipeline, learn_rules, inv_ids):
    violated_rules = {}
    # Process intervention videos
    for inv_id in inv_ids:
        # Process single observation
        report, violation_events = process_single_intervention_logic(
            pipeline, inv_id, learn_rules)
        violated_rules[inv_id] = violation_events
    return violated_rules


def consolidate_rules(observed_rules):
    # Consolidate learned rules from all observations
    rule_map = {}
    for obs_id, facts in observed_rules.items():
        for rule in facts["symbolic_facts"]:
            rule_str = f"{rule.head}(O):-{rule.body}(O)."
            if rule_str in rule_map:
                # Merge statistics for duplicate rules
                existing_rule = rule_map[rule_str]
                for stat in ['tp', 'fp', 'fn', 'tn']:
                    existing_rule.stats[stat] += rule.stats[stat]
            else:
                rule_map[rule_str] = rule

    learned_rules = list(rule_map.values())
    return learned_rules

def learn_rules(observed_rules):
    return observed_rules  # Placeholder for actual rule learning logic



def main():
    dataset_name = "dataset_mini"
    pipeline, observation_ids, inv_ids = initialize_pipeline(dataset_name)
    if pipeline is None:
        return
    ...
    for variant_obs_ids in observation_ids:
        # Iterate through each observation ID
        pipeline.update_variant_dir(variant_obs_ids)
        for obs_id in variant_obs_ids:
            # Process single observation
            processed_observation = process_single_observation_logic(
                pipeline, obs_id)
            observed_rules[obs_id] = processed_observation

        merged_rules = consolidate_rules(observed_rules)
        
        # reasoning and rating learned rules
        learned_rules = learn_rules(merged_rules)

        violated_rules = process_intervention_videos(
            pipeline, learned_rules, inv_ids)
...
def main_legacy():
    ...
    # dataset_name = "two_parts"
    dataset_name = "red_blue_colors"
    pipeline, observation_ids, inv_ids = initialize_pipeline(dataset_name)
    if pipeline is None:
        return
