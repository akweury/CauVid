# CauVid documentation

The repository documentation is grouped by subsystem so the project root can
remain focused on executable entry points and standard project configuration.

## Pipeline

- [`exp_july` pipeline steps](./pipeline/PIPELINE_STEPS.md)
- [ICLR 2027 research vision](./pipeline/PIPELINE_ICLR2027_RESEARCH_VISION.md)
- [Legacy video-processing pipeline guide](./pipeline/README_LEGACY_PIPELINE.md)

## `exp_august`

- [Living pipeline design specification](./exp_august/PIPELINE_DESIGN_SPEC.md)
- [Closed-loop design and notation](./exp_august/EXP_AUGUST_CLOSED_LOOP_FLOWCHART.md)
- [Step 7 diagnosis and bounded-proposal diagram](./exp_august/STEP7_DIAGNOSE_PROPOSE.pdf)
- [Step 8 bounded local re-estimation diagram](./exp_august/STEP8_LOCAL_REESTIMATION.pdf)
- [Archived legacy-pipeline review](./exp_august/KNOWN_ISSUES.md)
- [Executable pipeline guide](../src/exp_august/README.md)

## Annotation

- [Video segmentation annotation guide](./video_annotation.md)
